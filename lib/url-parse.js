...
stringify = function (parameters) {
  return parameters ? parameters.map(function (param) {
    ...
    try {
      param.key.replace(LANGUAGE_REGEX, decodeURIComponent)
      param.value.replace(LANGUAGE_REGEX, decodeURIComponent)
    }
    catch (e) { } // We are not handling anything.
    finally { key = param.key }

    try {
      if (param.value.match(LANGUAGE_REGEX)) {
        param.value = param.value && decodeURIComponent(param.value)
      }
    }
    catch (e) { } // We are not handling anything.
    finally { value = param.value }
