var url = require('url')
  ...
  , ENCODE_REGEX = /[!'()*]/g
  , OR_REGEX = '|'
  , CHINESEANDJAPANESE = '[\u0400-\u04FF]'
  , RUSSIAN = '[\u3400-\u9FBF]'
  , LANGUAGE_REGEX = CHINESEANDJAPANESE + OR_REGEX + RUSSIAN
  , stringify
  , parse
  , encode
  , percentEncode
...
stringify = function (parameters) {
  return parameters ? parameters.map(function (param) {
    ...
    try {
      if (param.key.test(new RegExp(LANGUAGE_REGEX))) {
        key = param.key && decodeURIComponent(param.key)
      }
    }
    ...
    try {
      if (param.value.test(new RegExp(LANGUAGE_REGEX))) {
