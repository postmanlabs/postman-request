...
export class MultiProtocolRequest extends EventEmitter implements http.ClientRequest {
  ...
  private readonly options: AutoRequestOptions
  ...
  constructor (options: AutoRequestOptions) {
    ...
    agent.createConnection(this, options, (err: Error | null, proto?: 'http1' | 'h2', req?: http2.ClientHttp2Session | http.ClientRequest ) => {
      if (err) {
        this.emit('error', err)
        return
      }
      if (proto === 'h2') {
        this.onHttp2(req as http2.ClientHttp2Session)
      }
      if (proto === 'http1') {
        this.onHttp(req as http.ClientRequest)
      }
    }, (socket) => {
  ...
  onHttp2 (connection: http2.ClientHttp2Session) {
    ...
    const options: RequestOptions = {...this.options, agent: {
