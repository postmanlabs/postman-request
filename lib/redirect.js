...
Redirect.prototype.onResponse = function (response) {
  const self = this
  const request = self.request
  const urlParser = request.urlParser
  const options = {}
  ...
  if (request.formData &&
    ...
    const formData = []
    const resetFormData = function (key, value, paramOptions) {
      // if `value` is of type stream
      if (typeof (value && value.pipe) === 'function') {
        // bail out if not a file stream
        if (!(Object.prototype.hasOwnProperty.call(value, 'fd') && value.path)) return
    ...
    for (let i = 0, ii = request.formData.length; i < ii; i++) {
      const formParam = request.formData[i]
